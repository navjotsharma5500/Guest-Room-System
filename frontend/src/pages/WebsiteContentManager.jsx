...
export default function WebsiteContentManager({ showToast = () => {} }) {
  ...
  const renderAbout = () => (
    <>
      {renderGeneric()}
      <SectionSettingsEditor sections={draft.sectionSettings || {}} onChange={(v) => setPath(["sectionSettings"], v)} />
      <Panel title="Mission">
        <div className="grid gap-4 md:grid-cols-2">
...
function FooterEditor({ draft, setPath }) {
  return <><Panel title="Footer"><div className="grid gap-4 md:grid-cols-2"><CmsImageUploader label="Footer Logo" folder="/public-guest-room/footer" value={draft.logo || ""} onChange={(v) => setPath(["logo"], v)} /><Field label="Footer Title" value={draft.title} onChange={(v) => setPath(["title"], v)} /><Field label="Footer Subtitle" value={draft.subtitle} onChange={(v) => setPath(["subtitle"], v)} /><Field label="Description" value={draft.description} onChange={(v) => setPath(["description"], v)} textarea /><Field label="Map URL / Google Maps Share Link" value={draft.mapUrl} onChange={(v) => setPath(["mapUrl"], v)} /><Field label="Quick Contact" value={draft.quickContact} onChange={(v) => setPath(["quickContact"], v)} /><Field label="Copyright Text" value={draft.copyrightText} onChange={(v) => setPath(["copyrightText"], v)} /><Field label="Maintained Text" value={draft.maintainedText} onChange={(v) => setPath(["maintainedText"], v)} /></div></Panel><LinksEditor title="Footer Quick Links" items={draft.quickLinks || []} onChange={(v) => setPath(["quickLinks"], v)} /><Panel title="Social Links"><div className="grid gap-4 md:grid-cols-2">{["instagram", "snapchat", "twitter", "facebook", "youtube"].map((key) => <Field key={key} label={key} value={draft.socialLinks?.[key]} onChange={(v) => setPath(["socialLinks", key], v)} />)}</div></Panel></>;
}
