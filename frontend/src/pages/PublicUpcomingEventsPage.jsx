...
export default function PublicUpcomingEventsPage() {
  ...
  return (
    <div className="upcoming-events-page">
      <style>{`
        .upcoming-events-page{min-height:100vh;background:linear-gradient(135deg,#eff6ff 0%,#fff 48%,#faf5ff 100%);color:#111827;font-family:'DM Sans',sans-serif}
        ...
        .upcoming-main{max-width:1200px;margin:auto;padding:64px 24px 72px}.upcoming-eyebrow{color:#c62828;font-size:12px;font-weight:700;letter-spacing:.16em;text-transform:uppercase}.upcoming-main h1{font-family:'EB Garamond',Georgia,serif;font-size:clamp(2.3rem,5vw,4rem);margin:10px 0 12px}.upcoming-intro{max-width:640px;color:#6b7280;line-height:1.7}
        .upcoming-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:20px;margin-top:40px}.upcoming-card{padding:24px;border:1px solid #e5e7eb;border-radius:18px;background:#fff;box-shadow:0 8px 26px rgba(15,23,42,.07);cursor:pointer;transition:transform .25s,box-shadow .25s}.upcoming-card:hover{transform:translateY(-4px);box-shadow:0 14px 38px rgba(15,23,42,.12)}
        .upcoming-card-head{display:flex;justify-content:space-between;gap:14px}.upcoming-society{color:#1d4ed8;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.08em}.upcoming-card h2{font-size:18px;line-height:1.35;margin:5px 0}.upcoming-badge{height:max-content;padding:5px 11px;border-radius:999px;background:#dbeafe;color:#1d4ed8;font-size:11px;font-weight:700}.upcoming-meta{display:grid;gap:9px;margin-top:18px;color:#4b5563;font-size:13px}.upcoming-meta span{display:flex;align-items:center;gap:8px}.upcoming-description{margin-top:17px;color:#6b7280;font-size:13px;line-height:1.65}
        ...
        {loading ? <div className="upcoming-empty">Loading upcoming events…</div> : error ? <div className="upcoming-empty">{error}</div> : upcomingEvents.length === 0 ? <div className="upcoming-empty">No events are scheduled for the upcoming week.</div> :
          <section className="upcoming-grid">{upcomingEvents.map((event, index) => {
            ...
            return <motion.article role="button" tabIndex={0} onClick={() => setSelectedEvent(event)} onKeyDown={(keyEvent) => { if (keyEvent.key === "Enter" || keyEvent.key === " ") setSelectedEvent(event); }} key={event._id || `${event.eventName}-${event.eventDate}-${index}`} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * .04 }} className="upcoming-card">
              ...
              <div className="upcoming-meta"><span><MapPin size={15} color="#c62828" />{venueLabel(event)}</span><span><CalendarIcon size={15} color="#2563eb" />{formatDate(event.eventDate)}{end !== event.eventDate ? ` to ${formatDate(end)}` : ""}</span><span><Clock size={15} color="#7c3aed" />{event.eventTime || "-"}{event.checkOutTime ? ` to ${event.checkOutTime}` : ""}</span><span><Users size={15} color="#15803d" />{event.societyName || "-"}</span></div>
