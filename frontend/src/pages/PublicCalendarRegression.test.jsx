...
test.each([
 ...
 expect(eventStatus(booking, date, minutes)).toBe(expected);
 expect(venueStatus(normalizeEvent(booking), date, minutes)).toBe(expected);
});
test.each([
 ['2026-09-11', 600, 'upcoming'], ['2026-09-11', 1400, 'active'],
 ['2026-09-12', 720, 'live'], ['2026-09-12', 1400, 'active'],
 ['2026-09-13', 600, 'active'], ['2026-09-13', 781, 'completed'],
])('multi-day %s minute %s => %s', (date, minutes, expected) => {
 const event = { ...booking, eventEndDate: '2026-09-13' };
 expect(eventStatus(event, date, minutes)).toBe(expected);
 expect(venueStatus(normalizeEvent(event), date, minutes)).toBe(expected);
});
...
test('C-Hall counts, selection, details, deduplication and minute refresh', async () => {
 jest.useFakeTimers();
 jest.setSystemTime(new Date(2026, 8, 11, 13, 0));
