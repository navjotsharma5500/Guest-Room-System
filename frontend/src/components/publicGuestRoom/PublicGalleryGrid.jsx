...
export default function PublicGalleryGrid({ images = [] }) {
    ...
    <>
      <div className="columns-1 gap-5 sm:columns-2 lg:columns-3">
        {images.map((image, index) => (
          <button key={`${image.image}-${index}`} onClick={() => setActive(image)} className="mb-5 block w-full overflow-hidden rounded-[2rem] border border-[var(--guest-border)] bg-white text-left shadow-lg">
            <img src={imgOrFallback(image.image || image.src || image.url)} alt={image.title || "Gallery"} className="w-full object-cover" />
            <div className="p-4">
              <p className="font-semibold text-[var(--guest-blue)]">{image.title || image.category}</p>
