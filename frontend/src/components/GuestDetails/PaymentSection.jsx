...
export default function PaymentSection({ b, theme, onPay }) {
  ...
  // ✅ USE BOOKING DATA DIRECTLY - Single Source of Truth
  const totalAmount = b.totalAmount || 0;
  const paidAmount = b.paidAmount || 0;
  const discount = b.discount || 0;
  const balanceAmount = b.balanceAmount || (totalAmount - paidAmount - discount);
  
  const isFullyPaid = balanceAmount <= 0 && totalAmount > 0;
  const hasPendingPayment = balanceAmount > 0 && paymentType === "Paid";
  ...
  return (
    <div className={`p-6 border-b ${theme === "dark" ? "border-gray-700" : "border-gray-200"}`}>
      <div className="col-span-2">
        ...
        <div className="flex items-center gap-4 flex-wrap">
          <p className={`font-semibold text-lg ${theme === "dark" ? "text-white" : "text-gray-900"}`}>
            {paymentType === "Free"
              ...
              : `Paid (₹${totalAmount.toLocaleString()})`}
          </p>

          {/* ✅ ALWAYS SHOW if there's pending payment */}
          {hasPendingPayment && !isDepartmentPayment && (
            <button
              onClick={onPay}
              className="ml-auto px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-medium flex items-center gap-2"
            >
              <Receipt size={16} />
              Make Payment
            </button>
          ...
          {totalAmount > 0 && (
            <span className={`px-3 py-1 rounded-full text-sm font-semibold ${
              ...
              : paidAmount > 0 ? "bg-yellow-100 text-yellow-700"
              : "bg-gray-100 text-gray-700"
            }`}>
              {isFullyPaid ? "✅ Fully Paid"
                : paidAmount > 0 ? `⚡ Partial (₹${paidAmount.toLocaleString()} / ₹${totalAmount.toLocaleString()})`
                : "⏳ Unpaid"}
            </span>
          )}
        </div>
        ...
        {isDepartmentPayment && hasPendingPayment && (
          <div className="mt-4 bg-gradient-to-r from-orange-50 to-amber-50 border-2 border-orange-400 rounded-xl p-5 shadow-md">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-bold text-orange-900 mb-1">💼 Department Payment Pending</h3>
                <p className="text-sm text-orange-700">This payment will be made by the department</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              <div className="bg-white/70 rounded-lg p-3 border border-orange-200">
                ...
                <p className="text-lg font-bold text-orange-900">₹{totalAmount.toLocaleString()}</p>
              </div>
              ...
              <div className="bg-white/70 rounded-lg p-3 border border-red-200">
                ...
                <p className="text-lg font-bold text-red-900">₹{balanceAmount.toLocaleString()}</p>
              </div>
            </div>

            <button
              onClick={onPay}
              className="w-full bg-gradient-to-r from-orange-600 to-red-600 text-white px-6 py-3 rounded-lg hover:from-orange-700 hover:to-red-700 transition font-bold shadow-lg hover:shadow-xl flex items-center justify-center gap-2"
            >
          ...
          <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="grid grid-cols-3 gap-4 text-sm">
              <div>
                ...
                <p className="text-blue-900 font-bold text-lg">₹{totalAmount.toLocaleString()}</p>
              </div>
              <div>
                <p className="text-green-700 font-medium">Paid So Far</p>
                <p className="text-green-900 font-bold text-lg">₹{paidAmount.toLocaleString()}</p>
              </div>
              <div>
                <p className="text-red-700 font-medium">Balance Due</p>
                <p className="text-red-900 font-bold text-lg">₹{balanceAmount.toLocaleString()}</p>
              </div>
            </div>
            {discount > 0 && (
              <div className="mt-2 pt-2 border-t border-blue-200">
                <p className="text-xs text-blue-600">Discount Applied: ₹{discount.toLocaleString()}</p>
              </div>
            )}
          </div>
        )}
